// train_pipeline.m — Pipeline-parallel Stories110M training on ANE
//
// Architecture: Double-buffered activations with overlapped forward/backward.
// Within a gradient accumulation batch (ACCUM_STEPS), weights are frozen,
// so Forward(step N) can overlap with Backward(step N-1).
//
// Pipeline flow per accumulation batch:
//   Step 0:   Forward(0)                        [prime the pipeline]
//   Step 1:   Backward(0) then Forward(1)       [overlap: dW(0) on CPU || ANE fwd(1)]
//   Step 2:   Backward(1) then Forward(2)       [overlap: dW(1) on CPU || ANE fwd(2)]
//   ...
//   Step N-1: Backward(N-2) then Forward(N-1)
//   Drain:    Backward(N-1)                     [drain the pipeline]
//
// The ANE is single-threaded, so backward ANE kernels and forward ANE kernels
// serialize naturally. The speedup comes from CPU dW gradient computations
// (cblas_sgemm dispatched to dw_q) overlapping with ANE forward calls.
//
// Double buffering: Two sets of activation buffers (acts[2][NLAYERS]).
// Forward step N writes to acts[buf], Backward step N-1 reads from acts[1-buf].
// A semaphore ensures backward finishes reading before forward overwrites.
//
// Additionally, backward needs data from forward's IOSurface outputs
// (kern[L].fwdAttn->ioOut, kern[L].fwdFFN->ioOut). Since forward for the
// next step will overwrite these, we save this data into per-buffer copies
// of the IOSurface data during forward.
//
// Build: make train_pipeline
// Run:   ./train_pipeline [--resume] [--steps N] [--lr F] [--data PATH]
...
// Per-layer saved IOSurface data for backward (what backward reads from
// fwdAttn->ioOut and fwdFFN->ioOut in the non-pipelined version).
// These are fp16 buffers matching the IOSurface layout.
typedef struct {
    _Float16 *fwdAttnOut;   // [6*DIM * SEQ] — Q,K,V,attn_out,xnorm plus o_out
    _Float16 *fwdFFNOut;    // [(2*DIM+3*HIDDEN) * SEQ] — ffn_out,h1,h3,silu,x2norm
} LayerIOSaved;

static LayerIOSaved layer_io_saved_alloc(void) {
    LayerIOSaved s;
    s.fwdAttnOut = (_Float16*)malloc(6*DIM*SEQ*sizeof(_Float16));
    s.fwdFFNOut  = (_Float16*)malloc((2*DIM+3*HIDDEN)*SEQ*sizeof(_Float16));
    return s;
}
static void layer_io_saved_free(LayerIOSaved *s) {
    free(s->fwdAttnOut); free(s->fwdFFNOut);
}

// Save IOSurface output data into buffer (so backward can read it later
// even after forward overwrites the IOSurface)
static void save_fwd_attn_io(LayerIOSaved *dst, IOSurfaceRef ioOut) {
    IOSurfaceLock(ioOut, kIOSurfaceLockReadOnly, NULL);
    memcpy(dst->fwdAttnOut, IOSurfaceGetBaseAddress(ioOut), 6*DIM*SEQ*sizeof(_Float16));
    IOSurfaceUnlock(ioOut, kIOSurfaceLockReadOnly, NULL);
}
static void save_fwd_ffn_io(LayerIOSaved *dst, IOSurfaceRef ioOut) {
    IOSurfaceLock(ioOut, kIOSurfaceLockReadOnly, NULL);
    memcpy(dst->fwdFFNOut, IOSurfaceGetBaseAddress(ioOut), (2*DIM+3*HIDDEN)*SEQ*sizeof(_Float16));
    IOSurfaceUnlock(ioOut, kIOSurfaceLockReadOnly, NULL);
}

// Write saved IO data back to an IOSurface for backward ANE kernels
static void restore_fwd_attn_io(IOSurfaceRef ioOut, const LayerIOSaved *src) {
    IOSurfaceLock(ioOut, 0, NULL);
    memcpy(IOSurfaceGetBaseAddress(ioOut), src->fwdAttnOut, 6*DIM*SEQ*sizeof(_Float16));
    IOSurfaceUnlock(ioOut, 0, NULL);
}
static void restore_fwd_ffn_io(IOSurfaceRef ioOut, const LayerIOSaved *src) {
    IOSurfaceLock(ioOut, 0, NULL);
    memcpy(IOSurfaceGetBaseAddress(ioOut), src->fwdFFNOut, (2*DIM+3*HIDDEN)*SEQ*sizeof(_Float16));
    IOSurfaceUnlock(ioOut, 0, NULL);
}
...
// =====================================================================
// Forward pass for one step, saving activations into acts[buf] and
// IOSurface snapshots into io_saved[buf].
// Returns loss and writes dlogits, probs, x_final, x_cur_out (last x_cur).
// =====================================================================
static float pipeline_forward(
    int buf,                        // which double-buffer slot (0 or 1)
    LayerActs acts[][NLAYERS],      // acts[2][NLAYERS]
    LayerIOSaved io_saved[][NLAYERS], // io_saved[2][NLAYERS]
    LayerKernels *kern,             // kern[NLAYERS]
    Kern *finalRmsKern, Kern *classifierKern, Kern *softmaxKern,
    float *embed, float *rms_final,
    float *x_cur,                   // scratch, DIM*SEQ
    float *x_final,                 // output: final hidden state (for classifier bwd)
    float *probs,                   // output: softmax probs
    float *dlogits,                 // output: loss gradient on logits
    uint16_t *input_tokens,
    uint16_t *target_tokens,
    double *t_ane, double *t_io, double *t_elem
) {
    uint64_t t0, t1;
    ...
    for (int L = 0; L < NLAYERS; L++) {
        LayerActs *ac = &acts[buf][L];
        memcpy(ac->layer_in, x_cur, SEQ*DIM*4);

        // Attention forward (ANE)
        t0 = mach_absolute_time();
        ...
        // Read activations from IOSurface into CPU buffers
        ...
        // Save full IOSurface output for backward (backward uses io_copy from it)
        save_fwd_attn_io(&io_saved[buf][L], kern[L].fwdAttn->ioOut);
        t1 = mach_absolute_time(); *t_io += tb_ms(t1-t0); t0 = t1;
        ...
        // Save full IOSurface output for backward
        save_fwd_ffn_io(&io_saved[buf][L], kern[L].fwdFFN->ioOut);
        t1 = mach_absolute_time(); *t_io += tb_ms(t1-t0); t0 = t1;
    ...
    // Final RMSNorm + Classifier + Softmax (all on ANE)
    t0 = mach_absolute_time();
    ...
    // NLL loss + gradient: dlogits = probs - one_hot(targets)
    float total_loss = 0;
    float invS = 1.0f / SEQ;
...
// =====================================================================
// Backward pass for one step, reading activations from acts[buf] and
// io_saved[buf]. Dispatches dW to dw_q/dw_grp for overlap.
// =====================================================================
static void pipeline_backward(
    int buf,                        // which double-buffer slot to read from
    LayerActs acts[][NLAYERS],
    LayerIOSaved io_saved[][NLAYERS],
    LayerKernels *kern,
    Kern *sdpaBwd2[],
    Kern *rmsAttBwd[], Kern *rmsFFNBwd[],
    LayerWeights *lw,
    LayerGrads *grads,
    float *rms_final,
    float *embed,
    float *grms_final, float *gembed,
    float *dlogits, float *probs,
    float *x_final,                 // saved from forward
    float *x_cur_bwd,              // scratch: last x_cur from forward (pre-final-rmsnorm input)
    uint16_t *input_tokens,
    uint16_t *target_tokens,
    // Backward scratch buffers (owned by caller)
    float *dy, float *dffn, float *dh1, float *dh3,
    float *dx_ffn, float *dx2, float *do_out_buf,
    float *dq, float *dk, float *dv, float *dx_attn,
    dispatch_queue_t dw_q, dispatch_group_t dw_grp,
    double *t_ane, double *t_io, double *t_elem, double *t_rms, double *t_cblas_wait
) {
    uint64_t t0, t1;
    ...
    // dembed async on CPU
    float *capt_dlogits = (float*)malloc((size_t)VOCAB*SEQ*4);
    memcpy(capt_dlogits, dlogits, (size_t)VOCAB*SEQ*4);
    ...
    // Final RMSNorm backward (CPU)
    {
        ...
        rmsnorm_bwd(dx_rms_final, grms_final, dy, x_cur_bwd, rms_final, DIM, SEQ);
        memcpy(dy, dx_rms_final, SEQ*DIM*4);
        free(dx_rms_final);
    }
    t1 = mach_absolute_time(); *t_rms += tb_ms(t1-t0);

    // Backward through 12 layers in reverse
    for (int L = NLAYERS-1; L >= 0; L--) {
        LayerActs *ac = &acts[buf][L];
        LayerGrads *gr = &grads[L];
        memcpy(dffn, dy, SEQ*DIM*4);

        // Restore IOSurface data from saved buffers (backward ANE kernels
        // read from fwdFFN->ioOut via io_copy)
        restore_fwd_ffn_io(kern[L].fwdFFN->ioOut, &io_saved[buf][L]);
        ...
        t1 = mach_absolute_time(); *t_ane += tb_ms(t1-t0);

        // dW FFN async (CPU — overlaps with subsequent ANE work)
        ...
        // Restore attention IOSurface data for backward ANE kernels
        restore_fwd_attn_io(kern[L].fwdAttn->ioOut, &io_saved[buf][L]);
...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        LayerGrads grads[NLAYERS]; LayerKernels kern[NLAYERS];

        // Double-buffered activations: acts[0][L] and acts[1][L]
        LayerActs acts[2][NLAYERS];
        // Double-buffered IOSurface snapshots
        LayerIOSaved io_saved[2][NLAYERS];
        ...
        for (int L = 0; L < NLAYERS; L++) {
            ...
            for (int b = 0; b < 2; b++) {
                acts[b][L] = layer_acts_alloc();
                io_saved[b][L] = layer_io_saved_alloc();
            }
        }
        ...
        if (!resuming) {
            printf("=== ANE Pipeline Training: Stories110M ===\n");
            printf("dim=%d hidden=%d heads=%d seq=%d vocab=%d layers=%d\n", DIM, HIDDEN, HEADS, SEQ, VOCAB, NLAYERS);
            printf("Pipeline parallelism: double-buffered forward/backward overlap\n");
            {
                printf("  Training from scratch (random init)\n");
                srand48(42);
                float scale_d=1.0f/sqrtf(DIM), scale_h=1.0f/sqrtf(HIDDEN);
                float out_scale = 1.0f/sqrtf((float)NLAYERS); // GPT-2 style: output projections scaled by 1/sqrt(N)
                for (int L=0; L<NLAYERS; L++) {
                    for(size_t i=0;i<WQ_SZ;i++){lw[L].Wq[i]=scale_d*(2*drand48()-1);lw[L].Wk[i]=scale_d*(2*drand48()-1);}
                    for(size_t i=0;i<WQ_SZ;i++){lw[L].Wv[i]=scale_d*(2*drand48()-1);lw[L].Wo[i]=scale_d*out_scale*(2*drand48()-1);}
        ...
        // Double-buffered forward outputs
        float *x_cur[2];       // scratch for forward pass current hidden state
        float *x_final[2];     // final hidden state (post final-rmsnorm)
        float *probs[2];       // softmax output
        float *dlogits[2];     // loss gradient on logits
        float *x_pre_final[2]; // x_cur at end of forward (pre final-rmsnorm, for bwd)
        for (int b = 0; b < 2; b++) {
            x_cur[b] = (float*)malloc(SEQ*DIM*4);
            x_final[b] = (float*)malloc(SEQ*DIM*4);
            probs[b] = (float*)malloc(SEQ*VOCAB*4);
            dlogits[b] = (float*)malloc(SEQ*VOCAB*4);
            x_pre_final[b] = (float*)malloc(SEQ*DIM*4);
        }

        // Double-buffered token pointers (saved for backward)
        uint16_t *saved_input_tokens[2] = {NULL, NULL};
        uint16_t *saved_target_tokens[2] = {NULL, NULL};
        ...
        // Final RMSNorm and classifier (recompiled per batch)
        ...
        double total_t_fwd=0, total_t_bwd=0, total_t_overlap=0;
        ...
        while (step < total_steps) {
            ...
            int kernels_needed = TOTAL_WEIGHT_KERNELS + 2*NLAYERS + 2; // layer + rmsnorm_bwd + classifier + finalRms
            ...
            // Compile final RMSNorm and classifier
            ...
            double batch_t_fwd=0, batch_t_bwd=0, batch_t_overlap=0;

            // ============================================================
            // PIPELINE LOOP
            // ============================================================
            // Determine how many steps in this accumulation batch
            int batch_steps = 0;
            for (int a=0; a<ACCUM_STEPS && step+a<total_steps; a++) batch_steps++;

            if (batch_steps == 0) break;

            // Pre-generate random positions for all steps in this batch
            size_t max_pos = n_tokens - SEQ - 1;
            size_t *positions = (size_t*)malloc(batch_steps * sizeof(size_t));
            for (int a=0; a<batch_steps; a++) positions[a] = (size_t)(drand48() * max_pos);

            int buf = 0;  // current double-buffer index

            // === Step 0: Prime the pipeline with Forward(0) ===
            {
                uint64_t t_fwd_start = mach_absolute_time();
                saved_input_tokens[buf] = token_data + positions[0];
                saved_target_tokens[buf] = token_data + positions[0] + 1;

                float loss = pipeline_forward(
                    buf, acts, io_saved, kern,
                    finalRmsKern, classifierKern, softmaxKern,
                    embed, rms_final,
                    x_cur[buf], x_final[buf], probs[buf], dlogits[buf],
                    saved_input_tokens[buf], saved_target_tokens[buf],
                    &t_ane, &t_io, &t_elem);

                // Save x_cur state for backward (input to final rmsnorm)
                // After the forward loop, x_cur holds the output of the last layer
                // We need to reconstruct it. The forward function modifies x_cur in place.
                // The last x_cur = acts[buf][NLAYERS-1].x2 + acts[buf][NLAYERS-1].ffn_out
                // Actually, pipeline_forward leaves x_cur with the final residual.
                // We need to save it before finalRmsKern overwrites context.
                // pipeline_forward uses x_cur as scratch; at the end it holds the
                // pre-final-rmsnorm hidden state. Save it.
                memcpy(x_pre_final[buf], x_cur[buf], SEQ*DIM*4);

                last_loss = loss;
                uint64_t t_fwd_end = mach_absolute_time();
                batch_t_fwd += tb_ms(t_fwd_end - t_fwd_start);
            ...
            // === Steps 1..N-1: Backward(prev) then Forward(cur) with overlap ===
            for (int a = 1; a < batch_steps; a++) {
                int prev_buf = buf;
                buf = 1 - buf;  // flip double buffer

                uint64_t t_step_start = mach_absolute_time();

                // --- Backward for previous step ---
                // The backward pass runs ANE kernels (serialized with everything)
                // and dispatches dW cblas_sgemm to dw_q (which overlap with future ANE work).
                uint64_t t_bwd_start = mach_absolute_time();
                pipeline_backward(
                    prev_buf, acts, io_saved, kern,
                    sdpaBwd2, rmsAttBwd, rmsFFNBwd,
                    lw, grads, rms_final, embed,
                    grms_final, gembed,
                    dlogits[prev_buf], probs[prev_buf],
                    x_final[prev_buf], x_pre_final[prev_buf],
                    saved_input_tokens[prev_buf], saved_target_tokens[prev_buf],
                    dy, dffn, dh1, dh3, dx_ffn, dx2, do_out_buf,
                    dq, dk, dv, dx_attn,
                    dw_q, dw_grp,
                    &t_ane, &t_io, &t_elem, &t_rms, &t_cblas_wait);
                uint64_t t_bwd_end = mach_absolute_time();
                batch_t_bwd += tb_ms(t_bwd_end - t_bwd_start);

                // --- Forward for current step ---
                // dW from the backward above may still be running on dw_q.
                // That is the overlap: CPU dW computation || ANE forward.
                // We do NOT wait for dw_grp here — the forward doesn't touch gradients.
                uint64_t t_fwd_start = mach_absolute_time();
                saved_input_tokens[buf] = token_data + positions[a];
                saved_target_tokens[buf] = token_data + positions[a] + 1;
                ...
                // Measure overlap: if dW tasks from backward are still running
                // during forward, the wall time < bwd + fwd.
                double step_wall = tb_ms(t_fwd_end - t_step_start);
                double step_bwd = tb_ms(t_bwd_end - t_bwd_start);
                double step_fwd = tb_ms(t_fwd_end - t_fwd_start);
                double overlap = (step_bwd + step_fwd) - step_wall;
                if (overlap > 0) batch_t_overlap += overlap;
            ...
            // === Drain: Backward for the last step ===
            {
                uint64_t t_bwd_start = mach_absolute_time();
                pipeline_backward(
                    buf, acts, io_saved, kern,
                    ...
                    lw, grads, rms_final, embed,
                    grms_final, gembed,
                    dlogits[buf], probs[buf],
                    x_final[buf], x_pre_final[buf],
                    saved_input_tokens[buf], saved_target_tokens[buf],
                    dy, dffn, dh1, dh3, dx_ffn, dx2, do_out_buf,
                    dq, dk, dv, dx_attn,
                    dw_q, dw_grp,
            ...
            total_batches++;
            total_t_fwd += batch_t_fwd;
            total_t_bwd += batch_t_bwd;
            total_t_overlap += batch_t_overlap;
            ...
            printf("  [batch %d: compile=%.0fms train=%.1fms (%.1fms/step) fwd=%.1fms bwd=%.1fms overlap=%.1fms compiles=%d]\n",
                   steps_batch, cms, tms, tms/steps_batch,
                   batch_t_fwd, batch_t_bwd, batch_t_overlap, g_compile_count);
        }
        ...
        printf("\n=== Pipeline Parallel Efficiency Report ===\n");
        ...
        printf("Avg train:       %.1f ms/step\n", total_train_ms / (total_steps_done > 0 ? total_steps_done : 1));
        printf("Forward total:   %.0f ms\n", total_t_fwd);
        printf("Backward total:  %.0f ms\n", total_t_bwd);
        printf("Overlap saved:   %.0f ms (%.1f%% of bwd)\n",
               total_t_overlap, total_t_bwd > 0 ? 100*total_t_overlap/total_t_bwd : 0);
        ...
        for (int L=0; L<NLAYERS; L++) {
            ...
            for (int b = 0; b < 2; b++) {
                layer_acts_free(&acts[b][L]);
                layer_io_saved_free(&io_saved[b][L]);
            }
        }
        free_kern(softmaxKern); free_kern(finalRmsKern); free_kern(classifierKern);
        ...
        for (int b = 0; b < 2; b++) {
            free(x_cur[b]); free(x_final[b]); free(probs[b]);
            free(dlogits[b]); free(x_pre_final[b]);
        }
    }
    return 0;
}
